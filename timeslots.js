  ...
  .on("end", async () => {
    ...
    if (wedointhis) {
      contacts.forEach((contact) => {
        ...
        const schoolName = contact['School'] === 'Unknown' ? 'Quaranteen University' : contact['School'];

        const schoolStartDate = new Date(contact['School Start Time UTC']);
        const schoolStartTime = date.format(schoolStartDate, 'hh:mm:ss A');
        const schoolStartTimeLocal = hasTimezone ? date.format(schoolStartDate.addHours(studentTimezone), 'hh:mm:ss A') : null;

        const studentStartDate = new Date(contact['Start Time UTC']);
        const studentStartTime = date.format(studentStartDate, 'hh:mm:ss A');
        const studentStartTimeLocal = hasTimezone ? date.format(studentStartDate.addHours(studentTimezone), 'hh:mm:ss A') : null;
